...
void handleMapCollision( entity_t* e, map_t* map )
{
	...
	// X COLLISION
	y_min_tile = (((UBYTE)e->pos.y.b.h-1 + e->bounds.top) >> 3) - 1;
	y_max_tile = (((UBYTE)e->pos.y.b.h-1 + 8) >> 3) - 1;
	f.w = e->pos.x.w + e->speed.x.w;
	if( e->speed.x.w > 0x7FFF )
	{
		x_tile = ((f.b.h + XOFFSET + e->bounds.left) >> 3) - 1;
	}
	else if( e->speed.x.w > 0 )
	{
		x_tile = ((f.b.h + XOFFSET - e->bounds.right) >> 3) + 1;
	}
	...
	{
		moving_up = 1;
		y_tile = (f.b.h - 16 + e->bounds.top) >> 3;
	}
